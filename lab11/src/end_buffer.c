...
void init_buffer(end_buffer_t *buf) {
    buf->count = 0;
...
void add_elem(end_buffer_t *buf, int elem) {
    buf->queue[buf->count] = elem;
    buf->count++;
}

int remove_elem(end_buffer_t* buf) {
    int elem = buf->queue[0];
    for (int i = 0; i < buf->count - 1; i++) {
        buf->queue[i] = buf->queue[i + 1];
    }
    buf->count--;
    return elem;
}
...
void* producer(void* params) {
    end_buffer_t *buf = (end_buffer_t*) params;
